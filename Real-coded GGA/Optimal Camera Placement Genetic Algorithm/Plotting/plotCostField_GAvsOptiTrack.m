...
%   'LogFile', 'MarkerSize', 'ViewAngle', 'SaveAs'.
    ...
    fig = figure('Name', sprintf('Per-point cost field — %s', ttStr), ...
        'Units', 'inches', ...
        'Position', [0.5, 0.5, sty.FigWidthFull, sty.FigHeightTall * 1.2], ...
    ...
    panels = struct( ...
        'unc',  {uncGA,  uncOpti}, ...
        'occ',  {occGA,  occOpti}, ...
        'name', {sprintf('GA-best (CF3: %.4f)', gaCost), ...
                 'OptiTrack ad-hoc'});
    ...
    for k = 1:2
        ...
        title(axU, sprintf('Uncertainty — %s (mean %.3f)', panels(k).name, ...
                            mean(panels(k).unc)), ...
        ...
        cb2 = colorbar(axO);
        cb2.Label.String = 'Per-point occlusion angle (°)';
        ...
        title(axO, sprintf('Occlusion — %s (mean %.1f°)', panels(k).name, ...
                            mean(panels(k).occ)), ...
            'FontSize', sty.FontSizeAxis, 'FontName', sty.FontName, ...
            'FontWeight', 'normal');
    ...
    sgtitle(sprintf('%s — Per-point cost field: GA-best vs OptiTrack (%s, sp = %.2f m)', ...
        ttStr, gmStr, opts.Spacing), ...
