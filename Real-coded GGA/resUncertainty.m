...
for  p =1:numPoints
    ...
    for i = 1:numCams
        ...
        if (u >= 1 && u <= resolution(1) && v >= 1 && v <= resolution(2))
            ...
            visible2Cams(end +1) = i;
        end

    if numel(visible2Cams)<2 %if there are less than 2 cameras then triangulation is not possible 
        uncertainties(p) = penaltyUncertainty; %point is not visible in any cameras FOV
        continue;
    end

    vertices = calcVertices(length(visibleCams), adjacentSurfaces, planes);
